...
print(f'{han.name} and {lou.name} are friends')
...
hans = Dog('Hans', '저먼 셰퍼드', 7)
print(f'{hans.name}의 인간 나이는 {hans.calculate_human_age()}살입니다.')
...
# 
# For example we can create a class called Agent. 
# An attribute of an Agent may be their height, eye color, name, etc. 
# A method is typically more similar to a function acting on the object itself, 
# for example having the Agent object print out its code name would be suitable for a method.
# 
...
# It is by convention named self. The real_name is the argument. 
...
# Note how we don't have any parenthesis after real_name, 
...
# For example, we could create the attribute **planet** for the Agent class. 
# Agents (regardless of their height,eye color,name, 
# or other attributes will always be on planet Earth, at least for now! 
...
a = Agent('AAA','White',180)


# In[20]:


print(f'm.real_name:{m.real_name}')
...
print(f'planet:{m.planet}')
print(f'planet:{a.planet}')
...
# also notice the difference in calling a method versus calling an attribute, 
# methods need you to call with a () at the end otherwise they won't actually be executed.
...
# The derived classes (descendants) override or 
...
class Person():
    ...
    def report(self):
        print("I(Person) am {} {}.".format(self.first_name,self.last_name))

    def hello(self):
        print("Hello!")


# ** Second Class will inherit from Person base class, 
# allowing it to inherit its attributes and methods. 
# Notice how we pass the class, we don't actually instantiate it with () , 
...
class Students():
    ...
    def __str__(self):
        return "we are {} person".format((self.names))
